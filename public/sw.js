// BSC Store Service Worker v2 — push notifications + offline caching

const SHELL_CACHE = 'bsc-shell-v2';
const API_CACHE   = 'bsc-api-v1';
const IMG_CACHE   = 'bsc-img-v1';
...
self.addEventListener('activate', e => {
  e.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(k => ![SHELL_CACHE,API_CACHE,IMG_CACHE].includes(k)).map(k => caches.delete(k))))
    ...
    e.respondWith(
      fetch(request).then(res => {
        ...
        if (cached) {
          ...
          return new Response(JSON.stringify({ ...data, _offline: true }), { headers: { 'Content-Type': 'application/json' } });
        }
